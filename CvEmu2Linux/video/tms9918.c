...
byte *New9918(tms9918 *VDP,byte *Buffer,int Width,int Height)
{
  ...
  VDP->VRAM=new byte [0x4000];
  
  if(!VDP->VRAM) return(0);
  
  //for (int i=0; i<0x4000; i++) VDP->VRAM[i]=0;
  ...
  if(!Buffer)
  {
    ...
    else       { delete (VDP->VRAM);return(0); }
  }

  /* Done */
  return((byte *) VDP->XBuf);
}
...
void Reset9918(tms9918 *VDP,byte *Buffer,int Width,int Height)
{
  ...
  if(Buffer)
  {
    if(VDP->OwnXBuf&&VDP->XBuf) delete (VDP->XBuf);
    VDP->XBuf=Buffer;
    VDP->Width=Width;
    VDP->Height=Height;
    VDP->OwnXBuf=0;
...
void Trash9918(tms9918 *VDP)
{
  ...
  if(VDP->VRAM)               delete (VDP->VRAM);
  if(VDP->XBuf&&VDP->OwnXBuf) delete (VDP->XBuf);
...
// Setta la palette per la visualizzazione
