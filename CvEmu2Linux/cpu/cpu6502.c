...
/* 
 ...
 Porting in c++ di un emulatore di procesore 6502
 Modifica effettuata da Giovanni Ortu
 
 Il file originale e' stato scaricato da http://howtoemulation.emuhq.com
 Non sono in grado di rintracciare l'autore originale del sorgente, il 
 file presente e' una "modifica della modifica" dell'emulatore 6502
 utilizzato in EMU scritto da Neil Bradley.
 
 Ecco quanto riportato sul sito
 
 "The original 6502 emulator that was used in EMU (which has long since been
  retired). Original source unknown, but Neil Bradley has tweaked it to run faster
  than the original."

 
 Lethian Software (Giovanni Ortu)
...
void rti6502()
{
      P=cvMemory[++S+0x100] | 0x20;
      PC=cvMemory[++S+0x100];
      PC |= cvMemory[++S+0x100] << 8;
}
...
//
// FIXED!!! SBC code for bcd operation
//
// Code from #START to #STOP Copyright (C) Marat Fayzullin 1996-2002  6502 emulator 
//
...
void sbc6502()
{
      ...
      unsigned char AL,AH,VL,VH,KH,KL;
...
void tsb6502()
{
      ...
      //cvMemory[savepc] |= A;
      if (cvMemory[savepc]) P &= 0xfd; else P |= 0x02;
}
...
void trb6502()
{
      ...
      //cvMemory[savepc] = cvMemory[savepc] & (A ^ 0xff);
...
/* Non uso put6502Mem e get6502Mem */

void nmi6502()
{
      cvMemory[0x0100+S--]=(BYTE)(PC>>8);
      cvMemory[0x0100+S--]=(BYTE)(PC & 0xff);
      cvMemory[0x0100+S--]=P;
      P |= 0x04;
      PC=cvMemory[0xfffa ];
      PC |= cvMemory[0xfffb ] << 8;
}
...
// Devo fare il check per sapere se gli interrupt sono mascherati
// Se il bit 2 di P e' a 1 allora gli interrupt sono mascherati
...
/* Creativision memory functions */
/* Implementare gli handler per le porte di IO */

inline int get6502memory(WORD addr)
{
   //***************************************
   // Intercetto l'IO sulle porte VDP e PIA
   //***************************************
   ...
   switch (addr & 0xf000)
   {
    case 0x1000: //Pia
     switch (addr & 0xf003)
     {
      case 0x1000: return pia_read(0,PIA_DDRA); break;      
      case 0x1001: return pia_read(0,PIA_CTLA); break;
      case 0x1002: return pia_read(0,PIA_DDRB); break;
      case 0x1003: return pia_read(0,PIA_CTLB) | 0x80; break;
      default:     break;
     }
     break;
     
    case 0x2000: case 0x3000: //Vdp
     switch (addr &  0xf001)
     {
      case 0x2000: cvMemory[VDPDATARD]=RdData9918(Vdp); break;
      case 0x2001: cvMemory[VDPCTRLRD]=RdCtrl9918(Vdp); break;
      case 0x3000: break;
      case 0x3001: break;
      default:     break;
     }
   ...
   return(cvMemory[addr]);
}

inline void put6502memory(WORD addr, BYTE byte)
{
   // Aggiorno il contenuto solo se e' ram 
   // e scrivo anche sulle parti mirror
   
   if (addr<0x4000)
    cvMemory[addr] = cvMemory[addr+0x0400]= cvMemory[addr+0x0800] = cvMemory[addr+0x0c00] =byte;
   ...
   switch (addr & 0xf000)
   {
    case 0x1000: //Pia*/
     switch (addr & 0xf003)
     {
      // Gestione strobe Tastiera (PA0..PA3) e Tape (PA4..PA7) (Scrittura)
      case 0x1000: pia_write(0,PIA_DDRA,byte); break;         
      case 0x1001: pia_write(0,PIA_CTLA,byte); break;
      case 0x1002: pia_write(0,PIA_DDRB,byte); break;
      case 0x1003: pia_write(0,PIA_CTLB,byte); break;
      default:     break;
     }
     break;
    case 0x2000: case 0x3000: //Vdp*/
     switch (addr &  0xf001)
     {
      case 0x2000: break;
      case 0x2001: break;
      case 0x3000: WrData9918(Vdp,cvMemory[VDPDATAWR]); break;
      case 0x3001: WrCtrl9918(Vdp,cvMemory[VDPCTRLWR]); break;
      default:     break;
     }
     break;
   }    // Pia Test & VDP Test*/
